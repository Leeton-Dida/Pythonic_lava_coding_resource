...
if(isset($_POST["id"]) && !empty($_POST["id"])){
    ...
    $stmt = $pdo->prepare("UPDATE quiz SET question=?,answer=?,correctAnswer=? WHERE id=?");

    if(!$stmt->execute([ $question,$answer,$correctAnswer,$id  ])) {
        echo "Something went wrong. Please try again later.";
        header("location: error.php");
    } else {
        $stmt = null;
        header("location: quiz-read.php?id=$id");
    }
} else {
        ...
        if($stmt = mysqli_prepare($link, $sql)){
            ...
            if(mysqli_stmt_execute($stmt)){
                ...
                if(mysqli_num_rows($result) == 1){
					...
					$correctAnswer = htmlspecialchars($row["correctAnswer"]);
