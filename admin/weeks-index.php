            ...
            <div class="row">
                <div class="col-md-12">
                    <div class="page-header clearfix">
                        <h2 class="float-left">Weeks Details</h2>
                        <a href="weeks-create.php" class="btn btn-success float-right">Add New Record</a>
                        <a href="weeks-index.php" class="btn btn-info float-right mr-2">Reset View</a>
                        <a href="index.php" class="btn btn-secondary float-right mr-2">Back</a>
                    </div>

                    <div class="form-row">
                        <form action="weeks-index.php" method="get">
                        <div class="col">
                    ...
                    $total_pages_sql = "SELECT COUNT(*) FROM weeks";
                    $result = mysqli_query($link,$total_pages_sql);
                    $total_rows = mysqli_fetch_array($result)[0];
                    $total_pages = ceil($total_rows / $no_of_records_per_page);

                    //Column sorting on column name
                    $orderBy = array('assignment_id', 'week_name');
                    $order = 'id';
                    ...
                    $sql = "SELECT * FROM weeks ORDER BY $order $sort LIMIT $offset, $no_of_records_per_page";
                    $count_pages = "SELECT * FROM weeks";


                    if(!empty($_GET['search'])) {
                        $search = ($_GET['search']);
                        $sql = "SELECT * FROM weeks
                            WHERE CONCAT_WS (assignment_id,week_name)
                            LIKE '%$search%'
                            ORDER BY $order $sort
                                ...
                                while($row = mysqli_fetch_array($result)){
                                    ...
                                    echo "<td>" . htmlspecialchars($row['assignment_id']) . "</td>";echo "<td>" . htmlspecialchars($row['week_name']) . "</td>";
                                        echo "<td>";
                                            ...
                                            echo "<a href='weeks-delete.php?id=". $row['id'] ."' title='Delete Record' data-toggle='tooltip'><i class='far fa-trash-alt'></i></a>";
                                        echo "</td>";
                                    echo "</tr>";
                                }
                                echo "</tbody>";
