...
if(isset($_POST["id"]) && !empty($_POST["id"])){
    ...
    $assignment_id = trim($_POST["assignment_id"]);
		$week_name = trim($_POST["week_name"]);
    ...
    $stmt = $pdo->prepare("UPDATE weeks SET assignment_id=?,week_name=? WHERE id=?");

    if(!$stmt->execute([ $assignment_id,$week_name,$id  ])) {
        echo "Something went wrong. Please try again later.";
        header("location: error.php");
    } else {
        $stmt = null;
        header("location: weeks-read.php?id=$id");
    }
} else {
        ...
        if($stmt = mysqli_prepare($link, $sql)){
            ...
            if(mysqli_stmt_execute($stmt)){
                ...
                if(mysqli_num_rows($result) == 1){
					...
					$week_name = htmlspecialchars($row["week_name"]);
            ...
            <div class="row">
                <div class="col-md-6 mx-auto">
                    ...
                    <form action="<?php echo htmlspecialchars(basename($_SERVER['REQUEST_URI'])); ?>" method="post">
                        ...
                        <a href="weeks-index.php" class="btn btn-secondary">Cancel</a>
                    </form>
                </div>
            </div>
        </div>
