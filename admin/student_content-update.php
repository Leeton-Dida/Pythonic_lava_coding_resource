...
if(isset($_POST["id"]) && !empty($_POST["id"])){
    ...
    $vars = parse_columns('student_content', $_POST);
    $stmt = $pdo->prepare("UPDATE student_content SET student_id=?,content_id=?,code=?,mark=? WHERE id=?");

    if(!$stmt->execute([ $student_id,$content_id,$code,$mark,$id  ])) {
        echo "Something went wrong. Please try again later.";
        header("location: error.php");
    } else {
        $stmt = null;
        header("location: student_content-read.php?id=$id");
    }
} else {
    ...
    if(isset($_GET["id"]) && !empty($_GET["id"])){
        ...
        $sql = "SELECT * FROM student_content WHERE id = ?";
        if($stmt = mysqli_prepare($link, $sql)){
            ...
            if(mysqli_stmt_execute($stmt)){
                ...
                if(mysqli_num_rows($result) == 1){
					...
					$content_id = htmlspecialchars($row["content_id"]);
            ...
            <div class="row">
                <div class="col-md-6 mx-auto">
                    ...
                    <form action="<?php echo htmlspecialchars(basename($_SERVER['REQUEST_URI'])); ?>" method="post">
                        ...
                        <a href="student_content-index.php" class="btn btn-secondary">Cancel</a>
                    </form>
                </div>
            </div>
        </div>
