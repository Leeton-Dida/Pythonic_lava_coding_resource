            ...
            <div class="row">
                <div class="col-md-12">
                    <div class="page-header clearfix">
                        <h2 class="float-left">Student assignments Details</h2>
                        <a href="student_assignment-create.php" class="btn btn-success float-right">Add New Record</a>
                        <a href="student_assignment-index.php" class="btn btn-info float-right mr-2">Reset View</a>
                        <a href="index.php" class="btn btn-secondary float-right mr-2">Back</a>
                    </div>

                    <div class="form-row">
                        <form action="student_assignment-index.php" method="get">
                            <div class="col">
                    ...
                    $total_pages_sql = "SELECT COUNT(*) FROM student_assignment";
                    $result = mysqli_query($link, $total_pages_sql);
                    $total_rows = mysqli_fetch_array($result)[0];
                    $total_pages = ceil($total_rows / $no_of_records_per_page);

                    //Column sorting on column name
                    $orderBy = array('student_id', 'assignment_id', 'code', 'submitted', 'date', 'mark');
                    $order = 'id';
                    ...
                    $sql = "SELECT * FROM student_assignment ORDER BY $order $sort LIMIT $offset, $no_of_records_per_page";
                    $count_pages = "SELECT * FROM student_assignment";


                    if (!empty($_GET['search'])) {
                        $search = ($_GET['search']);
                        $sql = "SELECT * FROM student_assignment
                            WHERE CONCAT_WS (student_id,assignment_id,code,submitted,date,mark)
                            LIKE '%$search%'
                            ORDER BY $order $sort
                    ...
                    if ($result = mysqli_query($link, $sql)) {
                        if (mysqli_num_rows($result) > 0) {
                            ...
                            while ($row = mysqli_fetch_array($result)) {
                                ...
                                echo "<td>" . htmlspecialchars($assignment_name) . "</td>";
                                ...
                                echo "<td>" . htmlspecialchars($row['submitted']) . "</td>";
                                ...
                                echo "<a href='student_assignment-delete.php?id=" . $row['id'] . "' title='Delete Record' data-toggle='tooltip'><i class='far fa-trash-alt'></i></a>";
                                echo "</td>";
                                echo "</tr>";
                            }
                            echo "</tbody>";
